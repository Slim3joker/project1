...
function defaultState() {
  return {
    ...
    ach: [],
    stats: { answered: 0, correctTotal: 0, bestCombo: 0, sessions: 0 },
  };
}
...
function viewIntro(session, item) {
  ...
  render(`
    ...
    <div class="introbtns">
      <button class="bigbtn secondary" id="s-known">✅ Kenne ich schon</button>
  ...
  $("#answers").querySelectorAll(".abtn").forEach((btn) => {
    btn.onclick = () => {
      ...
      if (correct) {
        ...
        session.gold += g;
        earnGold(g, $(".qcard"));
      } else if (isSRS && session.mode === "review") {
...
function viewDict() {
  render(`
    ...
    <div class="tabs" id="dtabs">
      ${["alle", "neu", "training", "gemeistert", "bekannt"]
